...
if [[ "${PROTOCOL,,}" = "panel" ]]; then
  ...
  readonly NameD=${CFG_CASE}_${PARAM_DIR_PATIENT}_vc
  readonly NameTD=${CFG_CASE}_${PARAM_DIR_PATIENT}_td
  readonly recalbamTD=${DIR_PANEL}/${NameD}_output.sort.realigned.fixed.recal.bam
  readonly mpileup=${DIR_PANEL}/${NameD}.mpileup
  readonly snpvcf=${DIR_PANEL}/${NameD}.output.snp.vcf
  readonly indelvcf=${DIR_PANEL}/${NameD}.output.indel.vcf

  ${BIN_MPILEUP} "${recalbamTD}" > "${mpileup}"
  ${BIN_VAR_SCAN} mpileup2snp "${mpileup}" --min-coverage "${CFG_VARSCAN_MPILEUP_MINCOVERAGE}" --min-var-freq "${CFG_VARSCAN_MINVAF}" \
    --min-freq-for-hom "${CFG_VARSCAN_MPILEUP_MINFREQFORHOM}" --min-avg-qual "${CFG_GENERAL_MINBASEQUAL}" --p-value "${CFG_VARSCAN_MPILEUP_PVALUE}" \
    --strand-filter "${CFG_VARSCAN_SOMATIC_STRANDFILTER}" --output-vcf 1 > "${snpvcf}"
  ${BIN_VAR_SCAN} mpileup2indel "${mpileup}"--min-coverage "${CFG_VARSCAN_MPILEUP_MINCOVERAGE}" --min-var-freq "${CFG_VARSCAN_MINVAF}" \
    --min-freq-for-hom "${CFG_VARSCAN_MPILEUP_MINFREQFORHOM}" --min-avg-qual "${CFG_GENERAL_MINBASEQUAL}" --p-value "${CFG_VARSCAN_MPILEUP_PVALUE}" \
    --strand-filter "${CFG_VARSCAN_SOMATIC_STRANDFILTER}" --output-vcf 1 > "${indelvcf}"

  names1="snp indel"
  for name1 in ${names1}
  do

    hc_vcf=${DIR_PANEL}/${NameD}.output.${name1}.vcf
    hc_avi=${DIR_PANEL}/${NameD}.output.${name1}.avinput
    hc_rci=${DIR_PANEL}/${NameD}.output.${name1}.readcount.input
    hc_rcs=${DIR_PANEL}/${NameD}.output.${name1}.readcounts
    hc_fpf=${DIR_PANEL}/${NameD}.output.${name1}.fpfilter.vcf

    ${CONVERT2ANNOVAR3} ${hc_avi}   ${hc_vcf}
    ${CUT}              ${hc_avi} > ${hc_rci}
    ${BamReadcount}  -l ${hc_rci}   ${recalbamTD} > ${hc_rcs}
    ${BIN_VAR_SCAN} fpfilter "${hc_vcf}" "${hc_rcs}" --output-file "${hc_fpf}" --keep-failures 1 \
      --min-ref-basequal "${CFG_GENERAL_MINBASEQUAL}" --min-var-basequal "${CFG_GENERAL_MINBASEQUAL}" \
      --min-var-count "${CFG_VARSCAN_FPFILTER_MINVARCOUNT}" --min-var-freq "${CFG_VARSCAN_MINVAF}" \
      --min-var-count-lc "${CFG_VARSCAN_FPFILTER_MINVARCOUNTLC}" --max-somatic-p "${CFG_VARSCAN_FPFILTER_MAXSOMATICP}" \
