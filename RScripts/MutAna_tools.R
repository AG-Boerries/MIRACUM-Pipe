...
div <- function(x_s, x_l, no_loh, protocol, capture_region){
  #' Mutation separation
  #'
...
mut_tab <- function(x_s_snp, x_s_indel, x_l_snp, x_l_indel, protocol){
  ...
  if (protocol == "wes"){
    ...
    muta_tab[1, 4] <- sum(x_s_snp$is_tumorsuppressor == 1 & x_s_snp$Zygosity == "hom")
    muta_tab[2, 4] <- sum(x_s_snp$is_tumorsuppressor == 1 & x_s_snp$Zygosity == "het")
    muta_tab[3, 4] <- sum(x_l_snp$is_tumorsuppressor == 1)
    muta_tab[4, 4] <- sum(x_s_indel$is_tumorsuppressor == 1 & x_s_indel$Zygosity == "hom")
    muta_tab[5, 4] <- sum(x_s_indel$is_tumorsuppressor == 1 & x_s_indel$Zygosity == "het")
    ...
    muta_tab[4, 5] <- sum(x_s_indel$is_oncogene == 1 & x_s_indel$Zygosity == "hom")
    muta_tab[5, 5] <- sum(x_s_indel$is_oncogene == 1 & x_s_indel$Zygosity == "het")
    ...
    muta_tab[4, 6] <- sum(x_s_indel$is_hotspot != 0 & x_s_indel$Zygosity == "hom")
    muta_tab[5, 6] <- sum(x_s_indel$is_hotspot != 0 & x_s_indel$Zygosity == "het")
  ...
  if (protocol == "panel") {
    ...
    muta_tab[1, 4] <- sum(x_s_snp$is_tumorsuppressor == 1 & x_s_snp$Zygosity == "hom")
    muta_tab[2, 4] <- sum(x_s_snp$is_tumorsuppressor == 1 & x_s_snp$Zygosity == "het")
    muta_tab[3, 4] <- sum(x_s_indel$is_tumorsuppressor == 1 & x_s_indel$Zygosity == "hom")
    muta_tab[4, 4] <- sum(x_s_indel$is_tumorsuppressor == 1 & x_s_indel$Zygosity == "het")
    
    muta_tab[1, 5] <- sum(x_s_snp$is_oncogene == 1 & x_s_snp$Zygosity == "hom")
    muta_tab[2, 5] <- sum(x_s_snp$is_oncogene == 1 & x_s_snp$Zygosity == "het")
    muta_tab[3, 5] <- sum(x_s_indel$is_oncogene == 1 & x_s_indel$Zygosity == "hom")
    muta_tab[4, 5] <- sum(x_s_indel$is_oncogene == 1 & x_s_indel$Zygosity == "het")
    
    muta_tab[1, 6] <- sum(x_s_snp$is_hotspot != 0 & x_s_snp$Zygosity == "hom")
    muta_tab[2, 6] <- sum(x_s_snp$is_hotspot != 0 & x_s_snp$Zygosity == "het")
    muta_tab[3, 6] <- sum(x_s_indel$is_hotspot != 0 & x_s_indel$Zygosity == "hom")
    muta_tab[4, 6] <- sum(x_s_indel$is_hotspot != 0 & x_s_indel$Zygosity == "het")
...
mut_stats <- function(x_s, x_l = NULL, tumbu, protocol) {
  ...
  print(paste("total number of mutations:", dim(x_s)[1] + dim_x_l, sep = " "))
  ...
  return(list(tot_mut <- totalmutationnumber, som_mut <- somaticmutations, loh_mut <- lohmutations))
}

tables <- function(x_s, x_l = NULL, protocol){
  #' Create Tables
  #'
  #' @description Create Tables for Tumorsuppressors/Oncogenes, all somatic
  ...
  return(list(ts_og_table = ts_og_table, sm_table = sm_table, lm_table = lm_table))
}
...
circos_colors <- function(x_s_snp = NULL, x_s_indel = NULL, x_l_snp = NULL,
                          ...
                          no_indel_somatic, no_snp, no_indel_loh, no_snp_loh){
  #' Circos Colors
  #'
  ...
  if (no_indel_somatic == FALSE & no_snp == FALSE & no_loh == FALSE
     & no_indel_loh == FALSE & no_snp_loh == FALSE){
  ...
  } else if (no_indel_somatic == FALSE & no_snp == FALSE & no_loh == FALSE
       & no_indel_loh == FALSE & no_snp_loh == TRUE){
  ...
  } else if (no_indel_somatic == FALSE & no_snp == TRUE & no_loh == FALSE
     & no_indel_loh == TRUE){
  ...
  } else if (no_indel_somatic == TRUE & no_snp == FALSE & no_loh == FALSE
     & no_indel_loh == FALSE){
...
omicCircosUni <- function(listOfMap, label = NULL, minR, outfile,
                          circosColors = NULL, protocol, capture_region) {
  #' omic Circos Uni
  #'
...
omicCircosFus2 <- function(listOfMap, fusions, label = NULL, minR, outfile,
                          circosColors = NULL, capture_region, protocol) {
  ...
  if (protocol != "Tumor_Normal" & mode %in% c("TruSight_Tumor", "TruSight_Amplicon", "Patho")){
    # Targeted Area Highlighting in CircosPlot 
    if (mode == "TruSight_Tumor"){
      trgt <- "/storage01/data/Annotation/Agilent/TrueSight_Tumor/TST170_DNA_target.bed"
    } else if (mode == "TruSight_Amplicon"){
      trgt <- "/storage01/data/Annotation/Agilent/TruSight_Amplicon/TSACP.bed"
    } 
    tg <- read.delim(file = trgt, header = FALSE)
    if (mode == "Patho"){
      tg <- tg[-c(1:2), ]
...
write_all_mut <- function(x_s, x_l = NULL){
  #' Write all Mutations
  #'
  #' @description Create lists of mutations
  #'
  #' @param x_s dataframe. List of somatic mutations
  #' @param x_l dataframe. List of LoH mutations
  #' 
  ...
  col_names <- c("Symbol", "GeneName", "ExonicFunc", "VAF", "Reads",
                 "AAChange", "CChange", "TSG", "OG", "HS", "target", "MAF",
                 "CADD13", "Condel", "CLINSIG", "COSMIC")
...
get_terms <- function(dataset, mut.entrez, t2.entrez){
  #' Pathway Analysis
  #'
...
write_mtb_genesets <- function(mulist, mtb.genesets){
  #' Important Pathways
  #' 
