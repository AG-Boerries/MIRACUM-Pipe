...
diagnosis <- as.character(args[26])
entity <- as.character(args[27])
gender <- as.character(args[28])
...
if (protocol == "panelTumor" & sureselect_type == "TSO500") {
  ...
  if (length(helper) == 1) {
    brcaness <- round(x = mut_sig_ana$output$Summary[helper, 3], digits = 2)
  } else {
    brcaness <- "NA"
  }
  biomarker <- data.frame(
    Tumor_Sample_Barcode = paste(as.character(sample), "TD", sep = "_"),
    MSI_SCORE = NA, #MSI_SCORE = ifelse(as.character(filt_result_td_mutect2$msi$MSI_status[1]) == "MSI-H",42,NA),
    MSI_TYPE = NA, #MSI_TYPE = ifelse(as.character(filt_result_td_mutect2$msi$MSI_status[1]) == "MSI-H","instable","stable"),
    Likely_POLE_deficiency= NA, #Likely_POLE_deficiency = as.character(filt_result_td_mutect2$msi$Likely_POLE_deficiency[1]),
...
if (protocol == "somaticGermline" | protocol == "somatic") {
  ...
  biomarker <- data.frame(
    Tumor_Sample_Barcode = paste(as.character(sample),"TD",sep = "_"),
    MSI_SCORE = NA,
    MSI_TYPE = NA, #as.character(filt_result_td$msi$MSI_status[1]),
    Likely_POLE_deficiency = NA, #as.character(filt_result_td$msi$Likely_POLE_deficiency[1]),
    CVR_TMB_SCORE = filt_result_td_10$tmb,
   BRCAness = brcaness
  )
  write.table(
    x = biomarker,
...
if (file.exists(fus_file)) {
  ...
  if (!is.null(fusions$Table)) {
    if (dim(fusions$Table)[1] != 0) {
      ...
      id_genes <- union(which(nchar(fus_tab$Chr1) > 2), which(nchar(fus_tab$Chr2) > 2))
      if (length(id_genes) > 0){
        fus_tab <- fus_tab[-id_genes, c("Gen1", "Chr1", "Pos1", "Gen2", "Chr2", "Pos2")]
      } else {
...
if (protocol == "somaticGermline"){
  ...
  sq_tab <- summary_quality(stats = stats)
  ...
  som_mut_pthw <- pthws_mut(df = mutation_analysis_result_10$important_pathways)
  som_mut_topart <- topart_mut(df = mutation_analysis_result_10$important_pathways)
  cnvs_pthws <- pathws_cnv(df = cnv_analysis_results$impa)

  germ_mut_cg <- highlight_detail(muts_tab = filt_result_gd$table, Mode = "Germline")
  germ_mut_pthw <- pthws_mut(df = mutation_analysis_result_gd$important_pathways)

  som_all <- highlight_detail(muts_tab = mutation_analysis_result$som_mut_tab, Mode = "Tumor")
  germ_all <- highlight_detail(muts_tab = mutation_analysis_result_gd$som_mut_tab, Mode = "Tumor")
  loh_all <- highlight_detail(muts_tab = mutation_analysis_result_10$table_loh_mutations, Mode = "LoH")
