...
filtering <- function(snpfile, indelfile, snpefffile_snp, snpefffile_indel,
                      outfile, outfile_maf, sample, path_data, path_script,
                      capture_region, covered_region, mode = "T", protocol = "wes", ) {
  #' Filter Variants
  #'
  #' @description Filters the SNPs and InDels for analysis
  #'
  ...
  #' @param capture_region string. .bed File of capture kit
  #' @param covered_region double. Covered region in Mb
  ...
  if (protocol == "panel") {
    x.snp <- read.delim(file = snpfile, sep = ",", header = TRUE)
    x.indel <- read.delim(file = indelfile, sep = "," , header = TRUE)
    x.snp <- x.snp[-c(1:19), ]
    x.indel <- x.indel[-c(1:19), ]
  }
  ...
  # Filter for targeted region in TST170
  if (capture_region == "panel"){
    x <- target_check(x, manifcapture_regionest)
  }
  ...
  x <- vrz(x = x, mode = mode, protocol = protocol, capture_region = capture_region)
  ...
  if (dim(x)[1] != 0) {
    ...
    x <- isogtsg(x, dbfile = paste(path_data, "CancerGenesList.txt",sep = "/"))
    x <- ishs(x, paste(path_data, "hotspots_V2.xls", sep = "/"))
    x <- isihs(x, paste(path_data, "hotspots_V2_.xls", sep = "/"))
    ...
    x <- oncokb(x, paste(path_data, "allActionableVariants.txt", sep = "/"))
  	...
  	x <- snpeff(x = x, sef_snp = snpefffile_snp, sef_indel = snpefffile_indel,
  	            protocol = protocol, capture_region = capture_region)
    ...
    x$Otherinfo <- gsub(x$Otherinfo, pattern = "\t", replacement = ";",
                        fixed = TRUE)
    x$target <- gsub(x$target, pattern = "\n", replacement = ", ",
                     fixed = TRUE)
