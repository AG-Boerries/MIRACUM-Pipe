...
mutation_analysis <- function(loh, somatic, tumbu, outfile_circos, path_data,
                             path_script, path_output,
                             protocol, capture_region, targets_txt){
  #' Mutation Analysis
  #'
  ...
  #' @param protocol string. Name of the anaylsis' protocol
  #'
  ...
  load(paste(path_data, "MTB_Genesets.RData", sep = "/"))
...
# split lists into sublists
  sub_lst <- div(x_s, x_l, no_loh, protocol, manifest)
# Write/print summary of mutationsnumber, diverse tables
  mutation_table <- mut_tab(sub_lst$x_s_snp, sub_lst$x_s_indel, sub_lst$x_l_snp, sub_lst$x_l_indel, protocol = protocol)
  if (!no_loh) {
