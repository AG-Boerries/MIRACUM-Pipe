...
<<eval=TRUE, results="tex", echo=FALSE, warning=FALSE>>=
df <- data.frame(Kriterium = c("Librarygr{\"o}{\ss}e", "Mean Coverage", "Basen > 8 Reads", "Basen > 40 Reads", "Exonische Basen > 8 Reads", "Exonische Basen > 40 Reads", "Insertgr{\"o}{\ss}e (Standardabweichung)", "QC-Anteil", "Mean Qualit{\"a}t pro Base"),
  Tumor = c(
...
kable(df, format = "latex", caption = "Qualit{\"a}tskriterien\\label{qual}", booktabs = T, row.names = F) %>% kable_styling(latex_options = c( "HOLD_position"), font_size = 9)
