...
tmp <- mutation_analysis_result$mutation_table
...
tmp.exon <- tmp[,2]
n_s <- sum(as.numeric(tmp.exon[c(1:2, 4:5)]))
n_l <- sum(as.numeric(tmp.exon[c(3,6)]))
n_a <- n_s + n_l
@
...
if (length(importantPathways) == 0){
  ...
  tbl <- importantPathways[ ,c('Pathway', 'Symbol', 'GeneName', 'ExonicFunc', 'VAF', 'Reads', 'AAChange', 'target', 'MAF', 'CADD', 'Condel', 'CLINSIG', 'COSMIC')]
