...
Im Anhang  befinden sich Tabellen zu allen detektierten somatischen, LoH und Keimbahnmutationen (Kap \ref{all_som_mut}, \ifthenelse{\equal{\detokenize{\Sexpr{modus}}}{\detokenize{SomaticGermline}}}{\ref{all_loh}}{}, \ref{all_germline}). Es werden allerdings nur seltene, exonische Mutationen mit einer VAF über 10\% gelistet.
...
if(dim(tmp)[1] == 0) {
  ...
  tmp$cosmic <- lapply(strsplit(x = tmp$cosmic84_coding, split = "="), function(x){return(unlist(x)[2])})
 ...
 tmp$ExonicFunc.refGene <- gsub(pattern = "nonframeshift insertion", replacement = "nfsIns", x = tmp$ExonicFunc.refGene)
  ...
  tmp$gnomAD_exome_NFE[is.na(tmp$gnomAD_exome_NFE)] <- "."
  ...
  kable(tmp, format = "latex", caption = cap_1, booktabs = T, longtable = T, digits = 2, row.names = F) %>% kable_styling(latex_options = c( "HOLD_position", "repeat_header"), font_size = 9) %>% column_spec(c(2:3, 8), width = "6em") %>% column_spec(c(1, 6), width = "4em") %>% column_spec(c(4:5), width = "4em") %>% column_spec(c(7, 9:10), width = "2em") %>% row_spec(0, angle = 60)
}
@
...
if(dim(mutation_analysis_result$table_loh_mutations)[1] != 0) {
 ...
 tmp$ExonicFunc.refGene <- gsub(pattern = "nonframeshift insertion", replacement = "nfsIns", x = tmp$ExonicFunc.refGene)
  tmp$ExonicFunc.refGene <- gsub(pattern = "frameshift insertion", replacement = "fsIns", x = tmp$ExonicFunc.refGene)
    ...
    tmp <- tmp[, c(1:2, 4, 3, 14, 13, 10:11, 17, 16, 15)]
...
if (length(tmp) == 0){
  ...
  tmp <- tmp[ ,c("Pathway", "Symbol", "GeneName", "AAChange", "ExonicFunc", "VAF", "Reads", "MAF", "Condel", "COSMIC84")]
  ...
  tmp$cosmic <- lapply(strsplit(x = tmp$COSMIC, split = "="), function(x){return(unlist(x)[2])})
  tmp$cosmic <- lapply(strsplit(x = as.character(tmp$cosmic), split = ";"), function(x){return(unlist(x)[1])})
  tmp$cosmic <- gsub(pattern = ",", replacement = ", ", x = tmp$cosmic)
...
%% bei MASTER
\ifthenelse{\equal{\detokenize{\Sexpr{modus}}}{\detokenize{SomaticGermline}}}{\subsection{Veränderungen in der Keimbahn}
...
if(length(grep(pattern = "MASTER", x = sample))>0){
  tmp <- filt_result_gd$table[, c("Gene.refGene", "GeneName", "AAChange", "ExonicFunc.refGene", "Variant_Allele_Frequency", "Variant_Reads", "gnomAD_exome_NFE", "condel.label", "cosmic84_coding", "is_oncogene", "is_tumorsuppressor")]
  ...
  if(dim(tmp)[1] != 0){
    ...
    tmp$AAChange <- gsub(pattern = ";", replacement = "; ", x = tmp$AAChange)
    tmp <- tmp[order(tmp$Variant_Allele_Frequency, decreasing =  TRUE), , drop = FALSE]
    tmp <- tmp[, c(1:4, 12, 7:8, 13, 10:11)]
    colnames(tmp) <- c("Gen", "Genname", "AA-Austausch", "Funktion", "VAF (Coverage)", "MAF", "Condel", "Cosmic", "OG", "TSG")
    cap_1 <- "Identifizierte Keimbahn Mutationen in Cancergenes.\\label{gl_cg}"
    kable(tmp, format = "latex", caption = cap_1, booktabs = T, longtable = T, digits = 2, row.names = F) %>% kable_styling(latex_options = c( "HOLD_position", "repeat_header"), font_size = 9) %>% column_spec(c(2:3, 8), width = "5em") %>% column_spec(c(1, 6), width = "5em") %>% column_spec(c(4:5), width = "3em") %>% column_spec(c(7, 9:10), width = "2em") %>% row_spec(0, angle = 60)
  } else {
...
%%
%% TEXT
%% include Explanation to table and additional tables that are available
%%
...
if(dim(tmp)[1] != 0){
  kable(tmp, format = "latex", caption = cap_1, booktabs = T, longtable = T, digits = 2, row.names = F) %>% kable_styling(latex_options = c( "HOLD_position", "repeat_header"), font_size = 9) %>% column_spec(c(2:3, 8), width = "6em") %>% column_spec(c(1, 6), width = "5em") %>% column_spec(c(4:5), width = "4em") %>% column_spec(c(7), width = "2em") %>% row_spec(0, angle = 60)
...
if(dim(mutation_analysis_result$table_loh_mutations)[1] != 0){
 ...
 tmp$ExonicFunc.refGene <- gsub(pattern = "nonframeshift insertion", replacement = "nfsIns", x = tmp$ExonicFunc.refGene)
  ...
  tmp$AAChange <- gsub(pattern = ";", replacement = "; ", x = tmp$AAChange)
  tmp <- tmp[order(tmp$VAF_tumor, decreasing =  TRUE), , drop = FALSE]
  tmp <- tmp[, c(1:2, 4, 3, 14, 13, 10:11, 15)]
  colnames(tmp) <- c("Gen", "Genname", "AA-Austausch", "Funktion", "VAF (Coverage) Tumor", "VAF (Coverage) Keimbahn", "MAF", "Condel", "Cosmic")
  if(dim(tmp)[1] != 0){
    kable(tmp, format = "latex", caption = "Identifizierte Loss of Heterozygosity geordnet nach VAF in der Tumorprobe", booktabs = T, longtable = T, digits = 2, row.names = F) %>% kable_styling(latex_options = c( "HOLD_position", "repeat_header"), font_size = 9) %>% column_spec(c(2, 9), width = "6em") %>% column_spec(c(1, 3), width = "5em") %>% column_spec(c(4:6), width = "3em") %>% column_spec(c(8), width = "2em") %>% row_spec(0, angle = 60)
  } else {cat("Keine Loss of Heterozygosity (LoH) identifiziert.")}
} else {cat("Keine Loss of Heterozygosity (LoH) identifiziert.")}
@

%%% NUR FÜR MASTER PATIENTEN!! %%%
\ifthenelse{\equal{\detokenize{\Sexpr{modus}}}{\detokenize{SomaticGermline}}}{\subsection{Keimbahn Mutationen}\label{all_germline}
<<echo=FALSE, eval=TRUE, results='tex'>>=
if (length(grep(pattern = "MASTER", x = sample)) == 0) {
  cat("\textbf{Es d\"urfen f\"ur diesen Patienten keine Keimbahnmutationen betrachtet werden.")
} else {
  tmp <- filt_result_gd$table[, c("Gene.refGene", "GeneName", "AAChange", "ExonicFunc.refGene", "Variant_Allele_Frequency", "Variant_Reads", "gnomAD_exome_NFE", "condel.label", "cosmic84_coding", "is_oncogene", "is_tumorsuppressor")]
  tmp <- tmp[-which(is.na(tmp$ExonicFunc.refGene)), ]
  tmp <- tmp[-grep(pattern = ";", x = tmp$Gene.refGene, fixed = TRUE), ]
  if(dim(tmp)[1] != 0){
    tmp$VAF <- paste0(tmp$Variant_Allele_Frequency, " (", tmp$Variant_Reads, ")")
    tmp$cosmic <- lapply(strsplit(x = tmp$cosmic84_coding, split = "="), function(x){return(unlist(x)[2])})
    tmp$cosmic <- lapply(strsplit(x = as.character(tmp$cosmic), split = ";"), function(x){return(unlist(x)[1])})
    ...
    cat("Keine Mutationen in der Keimbahn identifiziert.")
    }
}
@
}{}
