...
${BIN_MPILEUP} "${recalbam}" > "${mpileup}"
${BIN_VAR_SCAN} mpileup2snp "${mpileup}" --min-coverage "${CFG_VARSCAN_SOMATIC_MINCOVERAGE}" --min-var-freq "${CFG_VARSCAN_MINVAF}" --output-vcf 1 > "${snpvcf}"
${BIN_VAR_SCAN} mpileup2indel "${mpileup}" --min-coverage "${CFG_VARSCAN_SOMATIC_MINCOVERAGE}" --min-var-freq "${CFG_VARSCAN_MINVAF}" --output-vcf 1 > "${indelvcf}"
