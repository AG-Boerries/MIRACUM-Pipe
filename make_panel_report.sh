...
${BIN_RSCRIPT} "${DIR_RSCRIPT}/Main.R" "${CFG_CASE}" "${PARAM_DIR_PATIENT}" "${CFG_FILE_GERMLINE_R1}" "${CFG_FILE_TUMOR_R1}" \
  ...
  "${CFG_REFERENCE_CAPTURECORFACTORS}" "${CFG_PANEL_MINVAF}" "${CFG_PANEL_VARSCAN_FPFILTER_MINVARCOUNT}" "${CFG_GENERAL_MAFCUTOFF}" \
  "${CFG_REFERENCE_ACTIONABLEGENES}" "${CFG_REFERENCE_COVERED_EXONS}"
